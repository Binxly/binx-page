...
In simpler terms: every time you perform an operation on a tensor, Autograd is there, quietly taking notes. When it's time to update your model's parameters, it uses these notes to [calculate how each parameter needs to change](https://pytorch.org/tutorials/beginner/introyt/autogradyt_tutorial.html) to improve your model's prediction accuracy. This process is known as [backpropagation](<https://www.ibm.com/think/topics/backpropagation#:~:text=Backpropagation%20is%20a%20machine%20learning,(AI)%20“learn.”>).
...
As a simple example to illustrate how Autograd works, we can see it keep track of our model's gradients below:

![Google Colab - AutoGrad in Action - Calculating Gradients](https://i.imgur.com/OgFbhLB.png)

In this example:

1. We create a tensor `x` with `requires_grad=True`, which tells Autograd to track operations on this tensor.
2. We then perform some operation and finally call `backward()` on `z` to compute the gradients.
3. The result is stored in `x.grad`, which gives us the gradient of `z` with respect to `x`.
